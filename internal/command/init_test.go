...
// e.g. apply used here, before running an init operation to download the state storage provider.
func TestInit_uninitialized_stateStore(t *testing.T) {
	...
	t.Run("ok if working directory isn't initialized but state storage provider is supplied through reattach config", func(t *testing.T) {
		t.Skip("This is implemented as an E2E test")
	})
}
